...
def videofig(num_frames, redraw_func, play_fps=25, big_scroll=30, key_func=None,
  ...
  """
    ...
    scroll_handle = Slider(scroll_axes_handle, '', 0.0, num_frames - 1,
                           valinit=0.0)
    ...
    def scroll(new_f):
        new_f = min(max(new_f, 0),
                    num_frames - 1)  # clip in the range of [0, num_frames - 1]
...
def check_int_scalar(a, name):
    assert isinstance(a, int), '{} must be a int scalar, instead of {}'.format(
        name, type(name))
...
def check_callback(a, name):
    ...
    assert callable(a), '{} must be callable, instead of {}'.format(name,
                                                                    type(name))
